...
int mkstemps(char* tmpl, int suffixlen)
{
	...
	if (static_cast<size_t>(suffixlen) >= sizeof(saved_suffix))
		return -1;
	memcpy(saved_suffix, tmpl + placeholder_end, static_cast<size_t>(suffixlen));
	tmpl[placeholder_end] = '\0';

	errno_t err = _mktemp_s(tmpl, placeholder_end + 1);

	memcpy(tmpl + placeholder_end, saved_suffix, static_cast<size_t>(suffixlen));
	tmpl[len] = '\0';
